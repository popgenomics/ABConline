...
neutral = c(as.numeric(sfs[1]))
for(i in 2:length(sfs)){
	neutral = c(neutral, neutral[1]/i)
}
...
barplot(sfs, col=rev(viridis(1)), cex=1.5, cex.axis=1.5, cex.lab=1.5)
dev.off()
...
summary_modelComp = c(summary_modelComp, 'constant vs growth')
...
if(predicted_model_iso_mig$allocation=='migration'){
	if(predicted_model_Mhomo_Mhetero$allocation=='Mhetero'){
		...
		mod_cons_grow = abcrf(modIndexes~., data = data.frame(modIndexes, rbind(mig_ss, iso_ss)[, stats_sim]), ntree = ntree, paral = T, ncores = ncores)
		predicted_model_iso_mig = predict(mod_cons_grow, data.frame(obs_loci[, stats_obs]), training=data.frame(modIndexes, rbind(mig_ss, iso_ss)[, stats_sim]), ntree = ntree, paral = T, ncores = ncores)
