...
mean_exp = NULL
mean_obs = NULL
for(i in 2:ncol(gof2_ss)){
	stats = c(stats, colnames(gof2_ss)[i])
	pvals = c(pvals, round(pvalue(gof2_ss[,i], obs_ss[i]), 5))
	mean_exp = c(mean_exp, round(mean(gof2_ss[,i]), 5))
	mean_obs = c(mean_obs, round(as.numeric(obs_ss[i]), 5))
}
...
exp_sfs = NULL
for(i in 0:(nIterations_gof-1)){
	exp_sfs = rbind(exp_sfs, read.table(paste(timeStamp, '/gof_2/gof_', i, '/ABCjsfs.txt', sep=''), h=T))
